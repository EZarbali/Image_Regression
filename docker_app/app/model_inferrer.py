...
class Model_Inferrer:
    ...
    def _load_weights(self, model_path=None):
        ...
        model.regressor_backdoor_left.load_state_dict(
            torch.load("regressor_backdoor_left_weights.pth", map_location=device)
        )
        model.regressor_hood.load_state_dict(
            torch.load("regressor_hood_weights.pth", map_location=device)
        )
